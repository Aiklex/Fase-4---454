...
class AsesoriaEspecializada(Servicio):
    ...
    def validar_parametros(self, **kwargs):
        ...
        if nivel not in self.__tarifas_por_nivel:
            ...
            raise ParametroFaltanteError(
                f"Nivel '{nivel}' no es válido. Opciones: Junior, Senior, Expert."
            )

        if not isinstance(horas, (int, float)) or horas <= 0:
            raise ParametroFaltanteError(
                "La cantidad de horas debe ser un número mayor a 0."
            )
    ...
    def calcular_costo(self, nivel="Junior", horas=1, es_festivo=False):
        ...
        try:
            ...
            logger.info(
                f"Costo calculado para Asesoría ({self.__area}): ${base} (Nivel: {nivel})"
            )
            return base
    ...
    def __str__(self):
        return f"Asesoría '{self.nombre}' (Área: {self._AsesoriaEspecializada__area})"
