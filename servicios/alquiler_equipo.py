...
class AlquilerEquipo(Servicio):
    ...
    def validar_parametros(self, **kwargs):
        ...
        if (
            "dias" not in kwargs
            or not isinstance(kwargs["dias"], (int, float))
            or kwargs["dias"] <= 0
        ):
            logger.error(
                f"Validación fallida en AlquilerEquipo: {kwargs.get('dias')} días"
            )
            raise ParametroFaltanteError(
                "El número de días debe ser un valor numérico mayor a 0."
            )
    ...
    def calcular_costo(self, dias=1, incluir_seguro=False, descuento=0.0):
        ...
        try:
            ...
            logger.info(
                f"Cálculo de costo exitoso para Alquiler {self.__tipo_equipo}: ${total}"
            )
            return total
            ...
            raise SistemaFJError(
                f"Fallo inesperado al procesar el servicio: {e}"
            ) from e
    ...
    def __str__(self):
        return f"Equipo '{self.nombre}' (Tipo: {self._AlquilerEquipo__tipo_equipo})"
